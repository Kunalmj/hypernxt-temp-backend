  ...
  <div className="mb-5">
    ...
    <div className="relative">
      <select value={value} onChange={(e) => onChange(e.target.value)}
        className="w-full appearance-none bg-white border border-[#bfdbfe] rounded-xl px-3 py-2.5 text-[0.85rem] text-[#0f172a] font-medium cursor-pointer focus:outline-none focus:border-[#2563eb]">
...
>
        <div className="max-w-6xl mx-auto">
          ...
          <p className="text-blue-200 text-[0.85rem]">Explore 33 schemes in this category</p>
      ...
      <div className="max-w-6xl mx-auto px-4 md:px-6 py-8 flex gap-6 items-start">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between mb-5">
            ...
            <button onClick={() => setShowFilters((p) => !p)} className="inline-flex items-center gap-2 text-[0.8rem] font-semibold text-[#2563eb] border border-[#bfdbfe] bg-white rounded-xl px-4 py-2 cursor-pointer hover:border-[#2563eb] hover:text-[#2563eb] transition-colors" style={{ boxShadow: "0 1px 3px rgba(0,0,0,0.05)" }}>{showFilters ? "Hide Filters" : "Show Filters"}</button>
          </div>
          <div className="flex flex-col gap-4">
            {filtered.length === 0 ? (<div className="bg-white rounded-2xl p-10 text-center" style={{ boxShadow: "0 1px 3px rgba(0,0,0,0.06), inset 0 0 0 1px rgba(226,232,240,0.8)" }}><p className="text-[#2563eb] text-[0.9rem]">No schemes match your filters.</p></div>) : (
              filtered.map((scheme) => (
                <div key={scheme.id} className="bg-white rounded-2xl p-6 flex gap-4 transition-all duration-200" style={{ boxShadow: "0 1px 3px rgba(0,0,0,0.05), 0 4px 12px rgba(37,99,235,0.06), inset 0 0 0 1px rgba(226,232,240,0.8)" }}>
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[0.77rem] text-[#2563eb] mb-2">{scheme.ministry}</p>
