...
const StartupGrantResults = () => {
    ...
    fetchCategory("startup_msme/")
      .then((res) => {
        ...
        const mapped = rawResults.map((item, index) => {
          ...
          const stage = Array.isArray(item.stage) ? item.stage : (item.stage ? [item.stage] : ["Idea"]);

          return {
            id: item.id || `api-${index}`,
            title: item.title || "Untitled Scheme/Grant",
            provider: item.provider || "Government of India",
            amount: item.amount || "Refer to official website",
            minFunding: Number(item.min_funding) || 0,
            maxFunding: Number(item.max_funding) || 100000000,
            deadline: item.deadline || "Check official website",
            location: item.location || "PAN India",
            eligibility: item.eligibility || "Eligible startups/MSMEs",
            sector,
            stage,
            tags: Array.isArray(item.tags)
              ...
              : (typeof item.tags === "string"
                ...
                : ["Startup", ...stage].filter(Boolean)),
            website: item.url || "#",
            description: item.description || `${item.title} offered by ${item.provider || "Government of India"}.`,
            applicationMode: "Online",
            selectionProcess: "Screening -> Pitch -> Approval",
      ...
      .catch((err) => {
        console.error("Failed to fetch startup grants, falling back to local database:", err);
  ...
  const { filteredGrants, isExactMatch } = useMemo(() => {
    ...
    const matches = baseGrants.filter(grant => {
      // 1. Sector
...
>
        <div className="max-w-5xl mx-auto">
          ...
          <p className="text-lg text-blue-100 max-w-2xl opacity-90">
            {loading || error
              ? "Connecting to startup grant portals, please wait…"
              : isExactMatch 
      ...
      <div className="max-w-5xl mx-auto px-6 mt-6">
        ...
        <div className="grid gap-6">
          {loading ? (
            ...
            <div key={grant.id} className="bg-white rounded-2xl p-6 md:p-8 shadow-sm border border-slate-200 hover:shadow-md transition-shadow flex flex-col md:flex-row flex-wrap gap-6 md:items-start overflow-hidden">
              ...
              <div className="flex-1 min-w-0 space-y-4">
                ...
                <p className="text-sm text-slate-500 font-medium">{grant.provider}</p>
                <p className="text-slate-600 text-sm">
                  {expandedId === grant.id 
                    ? grant.description 
                    : (grant.description && grant.description.length > 180 
                      ? grant.description.slice(0, 180) + "..." 
                      : grant.description)}
                </p>
                ...
                {grant.website && grant.website !== "#" && (
                  <div className="mt-3">
                    <a href={grant.website} target="_blank" rel="noopener noreferrer" className="flex items-start gap-1.5 text-xs text-blue-600 hover:text-blue-800 font-semibold w-full">
                      ...
                      <span className="underline break-all flex-1 min-w-0">Official Website: {grant.website}</span>
                    </a>
                  </div>
                )}
              </div>
                ...
                <div className="w-full border-t border-slate-100 pt-6 mt-2 animate-fadeIn">
                  ...
                  <div className="mt-5 flex flex-col sm:flex-row items-center gap-3 justify-between bg-gradient-to-r from-blue-50 to-blue-50 rounded-xl p-4 border border-blue-100">
                    ...
                    >
                      ...
                      <span className="font-medium break-all flex-1 min-w-0">{grant.website || "Official website"}</span>
                    </a>
                    <button
                      onClick={() => {
                        setSelectedService(grant.title);
